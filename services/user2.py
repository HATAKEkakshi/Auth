...
class UserService:
    async def create_user(self, user_data: User,request:Request):
        ...
        cache_key= f"user2:{user_data.email}"
        # Email Checking 
        cached=await redis.get(cache_key)
        if cached:
            return {"message": "Email already exists", "Email": user_data.email}
        if not cached:
            existing_user = await user2_collection_name.find_one({"email": user_data.email})
            if existing_user:
                return {"message": "Email already exists", "Email": user_data.email}
            else:
                # Hash password
                ...
                cache_id_key=f"user2:id:{user_id}"
                user_dict['country'] = generate_country(user_dict['country_code'])
                # Save to DB
                await user2_collection_name.insert_one(user_dict)
    ...
    async def get_user(self, id: str,request:Request):
        redis=request.app.state.redis
        cache_key=f"user2:id:{id}"
        cached=await redis.get(cache_key)
        if cached:
            return json.loads(cached)
        user=await user2_collection_name.find_one({"id": id})
        if user:
    ...
    async def delete_user(self, id: str, email: str, request: Request):
        ...
        # Perform deletion
        await user2_collection_name.delete_one({"id": id})
